...
enum RoundaboutGeometry {
    static func direction(points: [GeoPoint], interval: ClosedRange<Int>) -> NavigationManeuver? {
        guard interval.lowerBound >= 0, interval.upperBound < points.count,
        ...
        // v4 may end the interval at the circle OR farther along the exit road.
        // Keep one extra segment when available; locate the actual outlet below.
        for index in start..<min(interval.upperBound + 1, points.count - 1) {
        ...
        for outletIndex in stride(from: segments.count - 1, through: 4, by: -1) {
            if let direction = direction(segments: segments, outletIndex: outletIndex) { return direction }
        }
        return nil
    }

    private static func direction(segments: [(heading: Double, meters: Double)], outletIndex: Int) -> NavigationManeuver? {
        let outlet = segments[outletIndex], last = segments[outletIndex - 1]
        guard outlet.meters >= 5, (15...140).contains(delta(last.heading, outlet.heading)) else { return nil }

        var sweep = 0.0, length = last.meters, count = 1
        for index in stride(from: outletIndex - 2, through: 0, by: -1) {
