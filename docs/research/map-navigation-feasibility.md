...
The current `tm/style.json` response was 113,407 bytes and selected one vector source with `maxzoom=15`. A z16 212×360 viewport at the H1 coordinate crosses four z15 parent tiles; their live response sizes were 340,726, 245,441, 280,880 and 218,121 bytes. Fetching one tile and clamping the camera to z15 was therefore geometrically incorrect. H1 now fetches the four required parent tiles in parallel, overzooms them by two, projects each tile from its own XYZ coordinate, clips at the z16 viewport and keeps only the `road` source layer.
...
Rendering those four live tiles through the native four-color road raster produced 1,842 bytes / 10 data chunks for 80 selected road segments and 3,440 bytes / 19 chunks for 200 segments. These are pre-hardware measurements using the exact 512-byte application envelope and remain below the 50–60 chunk rejection threshold.
