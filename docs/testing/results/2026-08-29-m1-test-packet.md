...
7. **Final ACK/result ordering:** With a deterministic sender fixture, begin final-step transmission, deliver its ACK and then the exact current-run OK result before resuming the final send continuation. Verify iOS buffers the result, reaches `displayed` only after the ACK, starts no result timeout and does not require reconnect. Repeat during begin or an earlier chunk and verify the OK is rejected as `ASSET_RESULT_INVALID` and never displayed.
8. **Render and replacement cleanup ownership:** Hold the Band file-delete callback after a render failure and verify no `ASSET_RENDER` result is sent and no new begin is admitted until that callback completes; then verify one explicit same-asset retry begins without `ASSET_BUSY` and makes exactly one provider call. Publish map A, then a different map B; verify A remains the fallback while B renders, B success is not advertised until A cleanup completes, and same-URI replacement never deletes the current map. If A deletion fails, verify A remains tracked and its cleanup is serialized before the next admitted begin rather than returning `ASSET_BUSY`; stale and duplicate callbacks must not publish twice or alter the confirmed map.
