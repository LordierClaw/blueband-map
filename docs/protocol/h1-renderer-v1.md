...
`render.result` (Band → iPhone) contains `runId`, `sceneId`, `renderer`, `formatVersion`, `success`, `bytes`, `primitives`, `validateMs`, `renderMs`, and optional `errorCode`. The Band sends one aggregate result rather than per-chunk logs.
